 ...
 */
field_t *
fieldread(file, delims, flags, maxf)
...
field_t *
fieldmake(line, allocated, delims, flags, maxf)
...
static field_t *
fieldparse(fieldp, line, delims, flags, maxf)
...
int maxf;                 /* Maximum number of fields to parse */
{
    ...
    if ((flags & FLD_NOSHRINK) == 0 && fieldp->nfields >= fieldmax) {
        fieldp->fields = (char **)realloc(
            fieldp->fields, (fieldp->nfields + 1) * sizeof(char *));
        if (fieldp->fields == NULL) {
            fieldfree(fieldp);
            return NULL;
        }
